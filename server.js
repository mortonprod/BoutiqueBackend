  ...
  if(!err) {
	        ...
	        if(count === 0 ){
	            ...
	            passwordsCollection.find({}).toArray(function(err,items){
	                ...
	                bcrypt.compare(req.body.password, items[0].hash, function(err, passed) {
	                    if(passed) {
	                        //req.session.login("Admin",req, function(err) {
	                        //    if (err) {
	                        //        return res.status(500).send("There was an error updating password. Please try again later.");
	                        //    }else{
                            //        req.session.userInfoss = "Admin";
                            //        console.log("Session after2: " + JSON.stringify(req.session));
    ...
    if(productsCollection != null){
        ...
        if(!req.body.price || req.body.price == "null" || req.body.price == ""){
            error.push("Product needs a price");
            isError = true;
        }
                ...
                productsCollection.count({'file':pathFile},function(err, count) {
                    if(count > 0){
                        ...
                        if(req.session.cookie.userInfo === "Admin"){
                            ...
                            res.send(["Need to sign in to admin"]);
                        }
                    }
                });
              }
	...
	app.post('/category', function(req,res,next){
	    console.log("Category uploaded: " +  JSON.stringify(req.body) + " session: " + JSON.stringify(req.session));
    ...
    /////
    session.Session.prototype.login = function(user,req, cb){
	    console.log("Session: " + JSON.stringify(this.req.session));
	    req.session.regenerate((err)=>{
	        if (err){
	            cb(err);
	        }else{
